...
describe('cds-grid-detail', () => {
  ...
  beforeEach(async () => {
    element = await createTestElement(html`
      <cds-grid style="width: 200px">
        ...
        <cds-grid-detail id="detail" hidden anchor="cell-1-action"></cds-grid-detail>
      </cds-grid>
    `);
  ...
  it('should align to the rowheader', async () => {
    ...
    expect(detailStyles.left).toBe('91px');
  });
  ...
  it('should align to the rowheader in RTL layouts', async () => {
    ...
    await componentIsStable(component);
    const detailStyles = getComputedStyle(component);
    expect(detailStyles.right).toBe('91px');
